...
def get_en_title_name(ko_game_url):
    ...
    res = session.get(en_game_url, headers={"Connection":"close"})
...
def get_metalink_from_google(searchfor):
    ...
    payload = {'q': searchfor}
    res = session.get(link, headers=header, params=payload)
...
def get_metainfo_from_metapage(metalink):
    res = session.get(metalink, headers=header)
    if res.status_code != 200: return ''
    ...
    try:
        ...
        metascore = -1
    try:
        userscore = float(userscore.get_text())
    except (ValueError, AttributeError):
        userscore = -1
